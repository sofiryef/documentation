...
The following is an example of adding OpenTelemetry's R2DBC instrumentation to the Datadog Java SDK:
...
mvn spring-boot:run -Dstart-class=com.baeldung.pagination.PaginationApplication \
  ...
  -Ddd.trace.otel.enabled=true -Dotel.javaagent.extensions=opentelemetry-javaagent-r2dbc.jar \
  -Ddd.trace.split-by-tags=db.name,db.sql.table -Ddd.trace.debug=true'
```
